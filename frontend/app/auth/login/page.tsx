...
export default function LoginPage() {
  ...
  const demoLogin = async () => {
    setLoading(true);
    try {
      const res = await api.post("/auth/login", { email: "john@example.com", password: "password123" });
      dispatch(setCredentials({ user: res.data.user, accessToken: res.data.accessToken }));
      toast.success(`Logged in as customer!`);
      router.push(res.data.user.role === "admin" ? "/admin" : redirect);
    } catch (err: any) {
      toast.error("Demo login failed. Run the seeder first: npm run seed");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-primary-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="card p-8 animate-scale-in">
          <div className="mb-6">
            <button onClick={demoLogin} disabled={loading} className="w-full btn-secondary text-sm py-2.5">
              👤 Demo Customer
            </button>
          </div>
